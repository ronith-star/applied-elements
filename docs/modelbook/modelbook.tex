...
Commit \texttt{765fc3d} \quad Built 2026-09-16\par}
\clearpage
...
Measured for this build by a full run: 1496 passed, 13 failed, 18 skipped, out of 1527 collected. The failures are confined to 2 prose-audit modules: \texttt{test\_test\_docstrings} (11), \texttt{test\_handoff\_claims} (2).
...
{\footnotesize\color{slate}Module \texttt{ae.plant.capacity}, 303 lines. Chapter text is this module's own documentation.\par}
...
Exactly 0.0 means two or more units bind simultaneously, in which case the \texttt{bottleneck} name is arbitrary (first in declaration order) and both must be expanded together to gain any capacity. Infinite for a single-unit line.
...
A 10 t/h mill at a feed-to-product ratio of 1.25 and an 8 t/h leach at 1.0 would instead give IDENTICAL product capacities. That is a tie, not a finding: the name returned is then merely first-in-declaration-order, and \texttt{bottleneck\_margin} is 0.0 to say so.
...
{\footnotesize\color{slate}Module \texttt{ae.plant.scheduling}, 459 lines. Chapter text is this module's own documentation.\par}
...
{\footnotesize\color{slate}Module \texttt{ae.econ.unit\_economics}, 330 lines. Chapter text is this module's own documentation.\par}
...
{\footnotesize\color{slate}Module \texttt{ae.econ.capex}, 530 lines. Chapter text is this module's own documentation.\par}
...
\begin{verbatimsmall}
reconciles(self, tol: float=1e-06) -> bool
\end{verbatimsmall}
...
{\footnotesize\color{slate}Module \texttt{ae.econ.valuation}, 445 lines. Chapter text is this module's own documentation.\par}
...
\begin{verbatimsmall}
irr(flows: np.ndarray | list[float], bracket: tuple[float, float]=(-0.9999, 10.0), tol: float=1e-10) -> fl
    \ oat | None
\end{verbatimsmall}

Internal rate of return, or \texttt{None} when it is not well defined.

Returns \texttt{None} when the stream never changes sign (no root exists) or when it changes sign more than once AND more than one root is actually found in the bracket, because reporting one of several roots as "the" IRR is misleading. Use \texttt{modified\_irr}, which is unique by construction.
...
{\footnotesize\color{slate}Module \texttt{ae.econ.uncertainty}, 549 lines. Chapter text is this module's own documentation.\par}
...
{\footnotesize\color{slate}Module \texttt{ae.ml.surrogate}, 556 lines. Chapter text is this module's own documentation.\par}
...
{\footnotesize\color{slate}Module \texttt{ae.agent.decisions}, 344 lines. Chapter text is this module's own documentation.\par}
...
lattice_al_ppm   EVPI   23.6244  as fraction  0.22970  switch fraction 0.3867
freight_usd_t    EVPI    8.2544  as fraction  0.09293  switch fraction 0.0000

freight_usd_t    cost     50.0  EVPI/cost 1.650872e-01
lattice_al_ppm   cost   2500.0  EVPI/cost 9.449758e-03

inner-sample convergence, upward bias falling as n_inner grows:
  n_inner   16: EVPI   33.2656
  n_inner   64: EVPI   32.9239
  n_inner  256: EVPI   32.6915
\end{outputblock}
...
57 doctest blocks in the module docstrings were executed, comprising 154 individual statements. 19 supplementary worked examples were executed, of which 19 ran without raising. Every number printed in a worked example in this book is the captured output of that execution.
...
24 case(s). These are defects in the repository's validation exporter, found while typesetting its output, not defects in the citations themselves: every DOI listed below resolves once the trailing characters are stripped, which was checked against the CrossRef REST API for each distinct DOI. The captured characters differ by case, most being sentence punctuation and one being a quote and backtick pair from an RST literal, and each entry names what was absorbed.
...
A full run collected 1527 tests: 1496 passed, 13 failed, 18 skipped, 0 errors. Provenance of these counts: commit \texttt{765fc3d}, recorded 2026-09-16T23:44:55, measured by this build: pytest was run over the whole suite and its junit XML parsed, at the time and commit recorded here. These counts are measured, not quoted from the task brief, and they differ from it: passed 1241 against 1496 measured; failed 148 against 13 measured; skipped 16 against 18 measured. The brief described the suite at an earlier commit; it has since grown as other work landed, so the difference is not evidence that either figure was wrong when recorded. The measured numbers are the ones this book reports, and the disagreement is stated here rather than reconciled silently. Failures by module: \texttt{test\_test\_docstrings} 11, \texttt{test\_handoff\_claims} 2. Every failure is in a prose-audit module; no model test fails.
